...
public class SoapServiceImpl implements SoapService50 {
   ...
   */
  private static final EnumMask<CommentFilter> COMMENT_FILTER_MASK =
      EnumMaskBuilder.with(CommentFilter.class)
          .add(
              CommentFilter.MUST_HAVE_COMMENT,
              CommentFilter.MUST_HAVE_RATING,
   ...
   * position of existing enums.
   */
  private static final EnumMask<CommentOrder> COMMENT_ORDER_MAPPING =
      EnumMaskBuilder.with(CommentOrder.class)
          .add(
              CommentOrder.REVERSE_CHRONOLOGICAL,
              CommentOrder.CHRONOLOGICAL,
              CommentOrder.HIGHEST_RATED,
  ...
  @Inject private ItemOperationFactory editFactory;
  @Inject private CloneFactory cloneFactory;
  @Inject private SoapXMLService soapXML;
  ...
  public String login(String username, String password) {
    return soapXML
        .convertUserToXML(userService.login(username, password, getDetails(), true).getUserBean())
        .toString();
  }
  ...
  public String loginWithToken(String token) {
    return soapXML
        .convertUserToXML(userService.loginWithToken(token, getDetails(), true).getUserBean())
        .toString();
  }
  ...
  public String newItem(String collectionUuid) {
    ...
    final PropBagEx item =
        soapXML.convertItemPackToXML(
  ...
  public String editItem(String itemUuid, int itemVersion, boolean modifyingAttachments) {
    ...
    final ItemPack pack =
        itemService.operation(
            getItemId(itemUuid, itemVersion), editFactory.startEdit(modifyingAttachments));
  ...
  public String newVersionItem(String itemUuid, int itemVersion, boolean copyAttachments) {
    final ItemPack pack =
        itemService.operation(
            getItemId(itemUuid, itemVersion), workflowFactory.newVersion(copyAttachments));
  ...
  public String cloneItem(String itemUuid, int itemVersion, boolean copyAttachments) {
    final ItemPack pack =
        itemService.operation(
            getItemId(itemUuid, itemVersion), cloneFactory.clone(copyAttachments, false));
  ...
  public void archiveItem(String itemUuid, int itemVersion) {
    itemService.operation(
        getItemId(itemUuid, itemVersion), workflowFactory.archive(), workflowFactory.save());
  }
  ...
  public String searchItems(
    ...
    DefaultSearch ls =
        createSearch(freetext, collectionUuids, whereClause, onlyLive, orderType, reverseOrder);
    ls.setSelect(is);
    ...
    for (Item item : results.getResults()) {
      if (item != null) {
        PropBagEx itemXml =
            soapXML.convertItemPackToXML(
                new ItemPack(item, itemService.getItemXmlPropBag(item), null), true);
  ...
  public String searchItemsFast(
    ...
    final SearchResults<ItemIdKey> results =
        freetextService.searchIds(
            createSearch(freetext, collectionUuids, whereClause, onlyLive, orderType, reverseOrder),
            offset,
            length);
  ...
  public int queryCount(String[] collectionUuids, String where) {
    return freetextService
        .countsFromFilters(Collections.singleton(createSearch(collectionUuids, where)))[0];
  }
  ...
  public String facetCount(
      String freetext, String[] collectionUuids, String whereClause, String[] facetXpaths)
      throws Exception {
    ...
    final ImmutableMap<String, String> mappedFacets =
        Maps.uniqueIndex(
            Arrays.asList(facetXpaths),
    ...
    final Multimap<String, Pair<String, Integer>> vcs =
        freetextIndex.facetCount(s, mappedFacets.keySet());
  ...
  private void itemOp(String itemUuid, int itemVersion, WorkflowOperation op) {
    ...
    itemService.operation(
        getItemId(itemUuid, itemVersion), workOps.toArray(new WorkflowOperation[workOps.size()]));
  }
  ...
  public String acceptTask(String itemUuid, int itemVersion, String taskId, boolean unlock) {
    itemService.operation(
        new ItemTaskId(itemUuid, itemVersion, taskId),
        workflowFactory.accept(taskId, null, null),
        workflowFactory.status(),
  ...
  public String rejectTask(
    ...
    itemService.operation(
        new ItemTaskId(itemUuid, itemVersion, taskId),
        workflowFactory.reject(taskId, rejectMessage, toStep, null),
        workflowFactory.status(),
        workflowFactory.saveUnlock(unlock));
  ...
  public String[] getItemFilenames(String itemUuid, int itemVersion, String path, boolean system) {
    ...
    final ItemFile fileHandle =
        itemFileService.getItemFile(new ItemId(itemUuid, itemVersion), null);
  ...
  public String getComment(String itemUuid, int itemVersion, String commentUuid) {
    Comment c =
        itemCommentService.getComment(
            itemService.getUnsecure(getItemId(itemUuid, itemVersion)), commentUuid);
  ...
  public String getComments(String itemUuid, int itemVersion, int filter, int order, int limit) {
    ...
    final List<Comment> comments =
        itemCommentService.getComments(item, filterEnums, orderEnum, limit);
  ...
  public void addComment(
      String itemUuid, int itemVersion, String commentText, int rating, boolean anonymous) {
